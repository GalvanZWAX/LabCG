...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// luz de faro azul
			glm::vec3 faroPos = glm::vec3(-3.78f + mainWindow.getmuevex(), 0.35f, -2.0f);
			glm::vec3 faroDir = glm::vec3(-1.0f, -0.5f, 0.0f);
		...
		model = glm::rotate(model, 90 * toRadians, glm::vec3(0.0f, -0.5f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = modelaux;
		model = glm::translate(model, glm::vec3(-0.4f, -1.0f, 2.7f));
		model = glm::rotate(model, 90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Llanta_M.RenderModel();
		...
		//blending siempre debe ser dibujado al ultimo 
		model = glm::mat4(1.0);
